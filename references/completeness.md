...
- `complete_scan: false` / `walk_complete: false` / `truncated: true` / `skipped_large` > 0 / `read_skipped_unreadable` > 0 → do not claim stub completeness (detail cap, files over 2 MB, unreadable files, or skipped walk entries were not fully read).
